...
class DM_rating_HR_Body extends StatefulWidget {
  @override

  final int rating;
  const DM_rating_HR_Body ({Key key, this.rating}):super(key:key);

  _DM_rating_HR_BodyState createState() => _DM_rating_HR_BodyState();
}

class _DM_rating_HR_BodyState extends State<DM_rating_HR_Body> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          title: Text("Delivery men with rating: ${widget.rating}",
            style: TextStyle(
        ...
        body: Padding(
          ...
          child: FutureBuilder(
            future: GetDeliveryMenSearchedRating(widget.rating),
            builder: (context, snapshot){
              if (snapshot.hasData)
              {
                dynamic DM_list = snapshot.data;
                if (DM_list.length==0)
                    Text("No delivery men with rating '$widget.rating'");
